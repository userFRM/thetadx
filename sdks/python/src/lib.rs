...
// ── Contract ID helper for Python dicts ──

/// Append contract identification fields to a Python dict.
/// Only adds them when expiration != 0 (i.e., wildcard query result).
    ...
    ($dict:expr, $tick:expr) => {
        if $tick.expiration != 0 {
            ...
            $dict
                ...
                .unwrap();
            $dict
                .set_item("strike_raw", $tick.strike)
                .unwrap();
            $dict
                .set_item(
                    "right",
